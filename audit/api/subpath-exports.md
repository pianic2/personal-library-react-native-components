...
| --- | --- | --- | --- | --- | --- |
| `@personal-library/react-native-components` | Small controlled root API for common consumer imports. | public | Approved public/beta components, providers, hooks, tokens and props types from `root-api-proposal.md`. | Should remain intentional and smaller than current `index.ts`; this proposal does not promote anything to stable. | HUMAN REVIEW REQUIRED |
| `@personal-library/react-native-components/components` | Public UI components grouped by component API. | public | `Button`, layout, typography, beta feedback, form, surface and navigation components plus their props types. | May include more than root, but should still exclude broken/internal modules; this proposal does not promote anything to stable. | HUMAN REVIEW REQUIRED |
...
| `@personal-library/react-native-components/tokens` | Token snapshots and token types. | public | `TokensSnapshot`, `TokenPair`; future neutral token names such as `themeTokens` and conditional `getThemeTokens`. | Legacy `auraTokens` / `getAuraTokens` are removed from the future API contract and must not be reintroduced as compatibility aliases. | HUMAN REVIEW REQUIRED |
...
| `@personal-library/react-native-components/legacy` | Deprecated package/import compatibility boundary, if separately approved. | deprecated | No AURA token aliases. | PLRNUI-16 rejects `auraTokens` and `getAuraTokens` compatibility aliases. | HUMAN REVIEW REQUIRED |
