...
- Core P1: Button now reads approved structural `theme.components.button` tokens after PLRNUI-30; Input now reads approved structural `theme.components.input` tokens after PLRNUI-31; Card still has a component-token contract that its implementation does not read.
