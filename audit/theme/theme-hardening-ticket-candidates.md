...
| PLRNUI-6-E | P1 | Split pure ThemeProvider from app-shell wrappers | ADR 0004 says safe area/scroll wrappers should be optional/separate; `ThemeProvider` currently renders `SafeAreaProvider`, `SafeAreaView`, and `ScrollView` by default. | Pure provider supplies only context and mode; optional app-shell wrapper is documented; migration note covers behavior change. |
| PLRNUI-6-F | P1 | Establish legacy AURA token deprecation path | `tokens/index.ts` exports `auraTokens`/`getAuraTokens`; `audit/api/export-matrix.md` marks them deprecated; `audit/migration/legacy-naming-map.md` leaves neutral names unknown. | Neutral token export names are approved; AURA aliases are either deprecated with removal milestone or moved to a legacy entrypoint; docs/migration notes updated. |
