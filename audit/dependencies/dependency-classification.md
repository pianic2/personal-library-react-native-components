...
| `expo-clipboard` | `^8.0.8` | `dependencies` | optional peer candidate; Expo-specific dependency; native dependency; dependency da verificare con PLRNUI-8 smoke test | Imported dynamically by `copyToClipboard`; should be optional because the package otherwise becomes Expo-specific. | `utils/clipboard.ts`; `package-lock.json` peer on `expo`, `react`, `react-native`; `audit/05-dependencies.md` DEP-06; ADR 0005 says Expo deps should be documented/isolated. | Non-Expo consumers need fallback or documented optional install. | Must verify module availability in Expo Go for the chosen SDK. | Managed workflow can use Expo module if version is SDK-compatible. | Prebuild/dev client required if native module is not supported by Expo Go baseline. | High; hard dependency makes Expo a package requirement. |
...
### DEP7-02 - Expo-specific clipboard is currently a hard runtime dependency

- Evidence: `package.json` puts `expo-clipboard` in `dependencies`; `utils/clipboard.ts` dynamically imports it; `package-lock.json` shows peers on `expo`, `react`, and `react-native`.
- Impact: the package can become Expo-specific even though ADR 0005 keeps React Native as the runtime target and only makes Expo the primary compatibility baseline.
- Risk: High.

### DEP7-03 - Native modules are mixed into runtime dependencies without optional peer policy

- Evidence: `@react-native-async-storage/async-storage`, `react-native-safe-area-context`, `react-native-svg`, `lucide-react-native`, and `expo-clipboard` are all in `dependencies`; ADR 0005 and Risk Assessment 0005 require native dependency control.
...
- Should clipboard support remain Expo-specific, become adapter-based, or move behind an optional utility/API?
