...
The library can typecheck, build and produce a package dry-run artifact. PLRNUI-42 resolves the current React peer mismatch against the selected latest stable Expo SDK baseline, and PLRNUI-43 aligns React Native package metadata to the approved `0.85.x` peer range. Release readiness still requires regenerated clean Expo consumer evidence, incomplete consumer import validation and native runtime compatibility proof.
